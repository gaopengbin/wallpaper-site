...
import { fetchMobileWallpaperList, fetchSearchSuggestions, getPreviewUrl, type WallpaperItem } from '../utils/api';

const MOBILE_CATEGORIES = [
  '美女', '风景', '动漫', '游戏', '明星', '简约', '文字',
  '暗黑', '可爱', '情侣', '二次元', '插画', '星空', '花卉',
  '萌宠', '创意', '摄影', '城市',
];
...
export default function Mobile() {
  ...
  const loadWallpapers = useCallback(
    async (pageNum: number, reset = false) => {
      ...
      const data = await fetchMobileWallpaperList({
        page: pageNum,
        rows: 50,
        lbName: category,
        sortType,
      ...
      if (data && data.list) {
        ...
        if (reset && data.list.length > 0) {
          const staticItems = data.list.filter((w) => w.type === 2);
  ...
  useEffect(() => {
    const onScroll = () => setHeroMode(window.scrollY < 80);
    ...
    <div style={{ paddingTop: 56, minHeight: '100vh', position: 'relative' }}>
      ...
      <div className="max-w-[1920px] mx-auto grid-container" style={{ padding: '0 24px 100px', position: 'relative', zIndex: 1 }}>
        {initialLoad ? (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '120px 0' }}>
            <div style={{
